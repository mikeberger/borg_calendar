...
public class WeekPanel extends ApptBoxPanel implements Navigator, Prefs.Listener,
	Printable {
	...
	for (int col = 0; col < 7; col++) {
	    ...
	    addDateZone(new DateZone(cal.getTime(), starthr * 60, endhr * 60),
		    colleft, 0, colwidth, calbot);
		...
		if (layout[col] == null && needLoad == true) {
			...
			if (appts != null) {
			    layout[col] = new ApptDayBoxLayout(cal.getTime(),
				    appts, starthr, endhr);
			}
		    }
		}
