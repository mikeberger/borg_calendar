...
public class CalendarPanel extends JPanel implements Prefs.Listener, Navigator, Model.Listener {
	...
	public DayMouseListener(MultiView mv, int year, int month, int date, boolean isDateButton) {
	    this.date = date;
	    this.year = year;
	    this.month = month;
	...
	try {
	    ...
	    navLabel_ = df.format(cal.getTime());
	...
	for (int i = 0; i < NUM_DAY_BOXES; i++) {
	    ...
	    // as per the experts, this subclass of JTextPane is the only
	    // way to
	    // stop word-wrap
	    JTextPane jep = null;
	    ...
	    // continue laying out the day panel. want the date button in
	    // upper
	    // right
	    ...
	    // scrollbars will only appear if needed due to amount of appt
	    // text
	...
	try {
	    ...
	    String marker = Prefs.getPref(PrefName.UCS_MARKER);
	    if (marker.indexOf('/') != -1) {
		s = textPane.addStyle("icon", bl);
		StyleConstants.setIcon(s, new ImageIcon(marker));
	    } else {
		URL icon = getClass().getResource("/resource/" + marker);
		if (icon != null) {
		    s = textPane.addStyle("icon", bl);
