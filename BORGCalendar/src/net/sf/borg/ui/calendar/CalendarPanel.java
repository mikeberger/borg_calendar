	...
	public void refresh() {
		try {
			...
			for (int i = 0; i < NUM_DAY_BOXES; i++) {
				...
				// invisible
				/*
				 * String show_extra = Prefs.getPref(PrefName.SHOWEXTRADAYS); if
				 * (!show_extra.equals("true") && (daynumber <= 0 || daynumber >
				 * ld)) { // the following line fixes a bug (in Java) where any //
				 * daytext not // visible in the first month would show its
				 * first line // of // text // in the wrong Style when it became
				 * visible in a // different // month - // using a Style not
				 * even set by this program. // once this happened, resizing the
				 * window would fix the // Style // so it probably is a swing
				 * bug addString(daytext[i], "bug fix", "black");
				 * days[i].setVisible(false); } else {
				 */
				...
				daytext[i].addMouseListener(new DayMouseListener(parent_, year,
						month, date, false));
				days[i].addMouseListener(new DayMouseListener(parent_, year,
						month, date, false));
				...
				daytext[i].setParagraphAttributes(daytext[i].getStyle("black"),
						true);
				if (appts != null) {
					...
					while (it.hasNext()) {
						...
						if (Prefs.getPref(PrefName.UCS_MARKTODO).equals("true")) {
							if (info.getTodo()
								...
								if (Prefs.getPref(PrefName.UCS_MARKER)
									...
									addString(daytext[i], Prefs
											.getPref(PrefName.UCS_MARKER), info
											.getColor());
								}
							}
						}
						...
						String apptText = di.getUntruncatedAppointmentFor(info)
								.getText();
						if (apptText.indexOf('\n') != -1) {
