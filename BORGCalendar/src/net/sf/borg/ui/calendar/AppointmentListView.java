...
public class AppointmentListView extends DockableView implements ListSelectionListener {
	...
	public Component getTableCellRendererComponent(JTable table, Object obj, boolean isSelected, boolean hasFocus, int row,
	    ...
	    Component c = defrend.getTableCellRendererComponent(table, obj, isSelected, hasFocus, row, column);
    ...
    static void onChangeDate(Component v, List appts) {
	...
	cal.set(dlgcal.get(Calendar.YEAR), dlgcal.get(Calendar.MONTH), dlgcal.get(Calendar.DAY_OF_MONTH));
    ...
    static void onMoveToFollowingDay(Component v, List appts) {
	// Move all selected ToDos to following day
	if (appts.size() == 0)
	    return;
	...
	apptTable.setModel(new TableSorter(new String[] { Resource.getResourceString("Text"), Resource.getResourceString("Time"),
		"Key" }, new Class[] { java.lang.String.class, java.util.Date.class, Integer.class }));
	...
	new PopupMenuHelper(apptTable, new PopupMenuHelper.Entry[] { new PopupMenuHelper.Entry(alDel, "Delete"),
	...
	add(jPanel3, gridBagConstraints21);
	...
	Date d = cal_.getTime();
	title_ = Resource.getResourceString("Appointment_Editor_for_") + DateFormat.getDateInstance(DateFormat.SHORT).format(d);
