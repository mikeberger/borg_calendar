...
public class MonthPrintPanel extends JPanel implements Printable {
	...
	if (!logo.equals("")) {
		...
		g2.drawImage(img, logoleft + ((2 * colwidth) - img.getWidth(null)) / 2, logotop + (rowheight - img.getHeight(null))
			/ 2, Color.WHITE, null);
	    }
	...
	try {
	    ...
	    drawIt(g, getWidth() / prev_scale, getHeight() / prev_scale, getWidth() / prev_scale - 20, getHeight() / prev_scale
		    - 20, 10, 10, 0);
	...
	Object options[] = { new Integer(1), new Integer(2), new Integer(3), new Integer(4), new Integer(5), new Integer(6),
		new Integer(7), new Integer(8), new Integer(9), new Integer(10), new Integer(11), new Integer(12) };
	Object choice = JOptionPane.showInputDialog(null, Resource.getResourceString("nummonths"), Resource
		.getResourceString("Print_Chooser"), JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
	if (choice == null)
	    return;
