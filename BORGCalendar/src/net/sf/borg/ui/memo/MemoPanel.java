...
public class MemoPanel extends JPanel implements ListSelectionListener,
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		if (isMemoEdited) {
			...
			// then do nothing - may be returning to the edited memo due to the
			// setSelectionInterval line
			// below that resets the selection
			if (editedMemoIndex == memoListTable.getSelectedRow())
				return;

			// selection is moving to a new memo - prompt about discarding
			// changes
			int ret = JOptionPane.showConfirmDialog(null, Resource
					.getResourceString("Edited_Memo"), Resource
					.getPlainResourceString("Discard_Text?"),
					JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);

			// if user does not want to lose changes, we need to set the
