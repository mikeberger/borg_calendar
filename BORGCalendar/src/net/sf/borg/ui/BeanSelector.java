...
public class BeanSelector extends JDialog {
    ...
    private Collection rows_ = new ArrayList(); // list of rows currently

    // displayed
    private static ArrayList list_ = new ArrayList();

    private String fields_[];
    private Collection records_ = null;

    public static Object selectBean(Collection records, TableModel tm, String fields[]) {
    ...
    public static Collection selectBeans(Collection records, TableModel tm, String fields[]) {
	new BeanSelector(records, tm, fields,true).setVisible(true);
	return list_;
    }

    private BeanSelector(Collection records, TableModel tm, String fields[], boolean multiple) {
	...
	while (it.hasNext()) {
		...
		for (int i = 0; i < fields_.length; i++) {
		    ...
		    Method m = beanClass.getDeclaredMethod(method, null);
		    ro[i] = m.invoke(r, null);
		}
		// add the table row
