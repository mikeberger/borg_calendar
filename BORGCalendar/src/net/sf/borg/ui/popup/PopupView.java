...
public class PopupView extends View {
    ...
    // find any popups that should no longer be displayed based on updates to the DB and delete them
    public void refresh() {
    ...
    static private boolean outside_reminder_times(long mins_to_go, Appointment appt) {
	...
	if( appt == null || appt.getDeleted()) return false;
	...
	if (AppointmentModel.isNote(appt) && appt.getTodo() && appt.getReminderTimes() != null
	...
	while (i.hasNext()) {
		...
		if (AppointmentModel.isNote(appt) && appt.getTodo()) {
		    ...
		    if (alarmid < 0) {
			time_msg = -alarmid + " " + Resource.getResourceString("minutes_ago");
		    } else if (alarmid == 0) {
			time_msg = Resource.getResourceString("Now");
		    } else {
			time_msg = alarmid + " " + Resource.getResourceString("minute_reminder");
		    }
		}
