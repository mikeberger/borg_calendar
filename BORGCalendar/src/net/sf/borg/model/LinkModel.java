...
public class LinkModel extends Model {
			...
			if (!f.exists()) {
				if (!f.mkdir()) {
					Errmsg.notice(Resource
							.getPlainResourceString("att_folder_err")
							+ path);
					return null;
				}
			}
	...
	public static String getName(Link at) throws Exception {
		if (at.getLinkType().equals(URL.toString()))
			...
			if (ap != null) {
				return (Resource.getPlainResourceString("appointment") + "["
						+ ap.getText() + "]");
			}
			...
			Project ap = TaskModel.getReference().getProject(
					Integer.parseInt(at.getPath()));
			if (ap != null) {
				return (Resource.getPlainResourceString("project") + "["
						+ ap.getDescription() + "]");
			}
			...
			Task ap = TaskModel.getReference().getTask(
					Integer.parseInt(at.getPath()));
			if (ap != null) {
				return (Resource.getPlainResourceString("task") + "["
						+ ap.getDescription() + "]");
			}
			...
			Address ap = AddressModel.getReference().getAddress(
					Integer.parseInt(at.getPath()));
			if (ap != null) {
				return (Resource.getPlainResourceString("Address") + "["
						+ ap.getLastName() + "," + ap.getFirstName() + "]");
			}

		} else if (at.getLinkType().equals(MEMO.toString())) {
			return (Resource.getPlainResourceString("memo") + "["
					+ at.getPath() + "]");
	...
	public void addLink(KeyedBean owner, String path, LinkType linkType)
			throws Exception {
		if (owner == null) {
	...
	public Collection getLinks() throws DBException, Exception {
		return db_.readAll();
	}
	...
	public void moveLinks(KeyedBean oldOwner, KeyedBean newOwner)
			throws Exception {
