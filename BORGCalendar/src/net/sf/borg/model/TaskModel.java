...
public class TaskModel extends Model implements Model.Listener, Transactional {
	...
	public Task getTask(int num) throws DBException, Exception {
	...
	private void addSubProjectsToCollection(Collection c, int projectid)
			throws Exception {
	...
	public void saveProject(Project p) throws Exception {
		...
		if (p.getId() != null && p.getId().intValue() != -1) {
			...
			Collection children = TaskModel.getReference().getSubProjects(
					p.getId().intValue());
