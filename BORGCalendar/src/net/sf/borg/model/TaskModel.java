...
public class TaskModel extends Model implements Model.Listener, Transactional, CategorySource {
    ...
    public void deleteProject(int id) throws Exception {
        ...
        try {
            if (db_ instanceof TaskDB == false)
                throw new Warning(Resource.getPlainResourceString("SubtaskNotSupported"));
    ...
    public void savetask(Task task) throws Exception {
        ...
        if (task.getProject() != null) {
            ...
            if (p != null && task.getDueDate() != null && p.getDueDate() != null
                    && DateUtil.isAfter(task.getDueDate(), p.getDueDate())) {
                throw new Warning(Resource.getPlainResourceString("taskdd_warning"));
            }
        }
        ...
        for (int i = 1;; i++) {
            ...
            if (ch.name().equals("OPTION")) {
                ...
                if (TaskModel.getReference().hasSubTasks()) {
                    ...
                    if (!isClosed(task) && task.getTodoList() != null && task.getUserTask1() != null) {
                        // add system subtasks
                        String todos = task.getTodoList();
                        String cbs[] = TaskModel.getReference().getTaskTypes().checkBoxes(task.getType());
...
*/
    ...
    public static int daysLeft(Date dd) {
        ...
        if (dcal.get(Calendar.YEAR) == today.get(Calendar.YEAR)) {
            ...
            days = new Long((dd.getTime() - today.getTime().getTime()) / (1000 * 60 * 60 * 24)).intValue();
        }
    ...
    public static int daysBetween(Date start, Date dd) {
        ...
        if (dcal.get(Calendar.YEAR) == startcal.get(Calendar.YEAR)) {
            days = dcal.get(Calendar.DAY_OF_YEAR) - startcal.get(Calendar.DAY_OF_YEAR);
        } else {
            days = new Long((dd.getTime() - startcal.getTime().getTime()) / (1000 * 60 * 60 * 24)).intValue();
        }
    ...
    public void saveProject(Project p) throws Exception {
        ...
        if (p.getId() != null && p.getId().intValue() != -1) {
            for( Task t : TaskModel.getReference().getTasks(p.getId().intValue()))
            {
                if (p.getDueDate() != null && t.getDueDate() != null && !TaskModel.isClosed(t)
            ...
            for( Project child : TaskModel.getReference().getSubProjects(p.getId().intValue()))
            {
                if (p.getDueDate() != null && child.getDueDate() != null && !TaskModel.isClosed(child)
        ...
        if (p.getId() != null && p.getStatus().equals(Resource.getPlainResourceString("CLOSED"))) {
            ...
            for( Task pt : TaskModel.getReference().getTasks(p.getId().intValue()))
            {
                if (!isClosed(pt)) {
