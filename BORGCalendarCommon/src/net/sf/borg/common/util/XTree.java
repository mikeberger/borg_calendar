...
public class XTree {
	...
	private static Token get_token(Reader r, StringBuffer buf,
			boolean open_found) throws Exception {
	...
	private static XTree parse_xml(Reader r) throws Exception {
		...
		while (true) {
			...
			// found an empty element
				...
				// degenerate case - if the tree is empty, the whole tree is
				// going
			...
			// got element data - set the current element's value
				...
				if (tree == null) {
					throw new Exception(
							"Illegal non-whitespace before XML start");
				}
