...
public class TaskListView extends View {
	...
	private void initComponents() {
		...
		tp.addTab(Resource.getPlainResourceString("projects"),
				getProjectPanel());
	...
	public void refresh() {
		...
		try {
			...
			while (ti.hasNext()) {
				...
				/*
				 * if
				 * (statfilt.equals(Resource.getPlainResourceString("All_Open"))) { //
				 * System.out.println(type + " " + //
				 * TaskModel.getReference().getTaskTypes().getFinalState(type));
				 * if (st.equals(TaskModel.getReference().getTaskTypes()
				 * .getFinalState(type))) { continue; } } else if
				 * (!statfilt.equals(Resource .getPlainResourceString("All")) &&
				 * !statfilt.equals(st)) continue;
				 */
				// category
