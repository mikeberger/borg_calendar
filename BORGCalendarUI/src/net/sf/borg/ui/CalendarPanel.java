		...
		try {
			...
			for (int i = 0; i < NUM_DAY_BOXES; i++) {
				...
				if (!show_extra.equals("true")
					...
					daynum[i].addMouseListener(new DayMouseListener(parent_,year,
							month, date, true));
					daytext[i].addMouseListener(new DayMouseListener(parent_,year,
							month, date, false));
					days[i].addMouseListener(new DayMouseListener(parent_,year, month,
							date, false));
