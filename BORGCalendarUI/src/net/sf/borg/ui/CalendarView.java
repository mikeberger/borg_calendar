...
public class CalendarView extends View implements Prefs.Listener {
	...
	private void init() {
		...
		/*
		 * dbInfo = new JTextPane() { public boolean
		 * getScrollableTracksViewportWidth() { return false; }
		 * 
		 * public void setSize(Dimension d) { if (d.width <
		 * getParent().getSize().width) { d.width = getParent().getSize().width; }
		 * super.setSize(d); } }; dbInfo.setBackground(new Color(204, 204,
		 * 204)); dbInfo.setEditable(false); JScrollPane sp = new JScrollPane();
		 * sp.setViewportView(dbInfo); sp
		 * .setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		 * sp
		 * .setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
	...
	private void AboutMIActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_AboutMIActionPerformed
		...
		String version = "";
		try {
			// get the version and build info from a properties file in the jar
			// file
