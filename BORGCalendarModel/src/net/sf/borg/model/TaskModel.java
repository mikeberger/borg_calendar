...
public class TaskModel extends Model implements Model.Listener {
	...
	private void load_map() {
		...
		try {
			...
			while( ti.hasNext() ) {
				...
				if( status.equals(tt.getFinalState(mr.getType())) || status.equals("PR"))
					continue;
	...
	public void savetask(Task task) throws Exception {
		...
		if( num.intValue() == -1 ) {
			...
			try {
				// update close date
				if( task.getState() != null && task.getState().equals(TaskModel.getReference().getTaskTypes().getFinalState(task.getType())))
					task.setCD(new Date());
