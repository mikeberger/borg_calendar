...
import net.sf.borg.model.db.DBException;
 ...
 // So, the schema for MDB is simply: int key; short userflags; - some boolean flags that are indexed text userdata 
...
public class MDB {
	...
	private void add(int key, int flags, String st, boolean crypt,
		...
		for (int extnum = (ulen - 1) / text_per_block; extnum >= 0; extnum--) {
			...
			try {
				...
				throw new DBException("add: seek failed",
	...
	private String read(int key, boolean system) throws DBException {
		...
		int bnum = find_block(key, system);
		if (bnum == -1)
			throw new DBException("find_block failed: "
					+ Integer.toString(key), DBException.RET_NOT_FOUND);
