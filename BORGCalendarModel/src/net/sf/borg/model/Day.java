...
public class Day {
	...
	// get Day Class for a given day. indicate if public or private appts are to
	// be included
	// The Day class is used by Views that need to present an entire day at a
	// time.
	...
	public static Day getDay(int year, int month, int day, boolean pub,
		...
		if (users != null) {
			...
			while (mumit.hasNext()) {
				...
				if (l != null) {
					...
					// iterate through the day's appts
					while (it.hasNext()) {
						String tx = "[" + user + "] ", txFull = "[" + user
								+ "] ";
						Integer ik = (Integer) it.next();

						// read the appt from the DB
						appt = otherModel.getAppt(ik.intValue());

						// skip based on private flag
						if (appt.getPrivate()) {
							continue;
						}
						...
						if (!AppointmentModel.isNote(appt) && prependTime) {
							Date d = appt.getDate();
							SimpleDateFormat sdf = AppointmentModel
									.getTimeFormat();
							tx += sdf.format(d) + " ";
							txFull += sdf.format(d) + " ";
						}
						...
						// add repeat number
						if (Repeat.getRptNum(appt.getFrequency())) {
							Calendar cur = new GregorianCalendar(year, month,
									day);
							tx += " ("
									+ Repeat.calculateRepeatNumber(cur, appt)
									+ ")";
							txFull += " ("
									+ Repeat.calculateRepeatNumber(cur, appt)
									+ ")";
		...
		if (l != null) {
			...
			while (it.hasNext()) {
				...
				String de = "BT" + task.getTaskNumber().toString() + " "
						+ task.getDescription();
